...
fun generateKotlinPluginArtifactFile(rootProject: Project): PFile {
    ...
    val kotlincDirectory = Directory("kotlinc").also { root.add(it) }

    run copyKotlinc@ {
        val kotlincBaseDir = File(rootProject.projectDir, "dist/kotlinc")

        for (file in kotlincBaseDir.walk().filter { it.isFile }) {
            val path = file.parentFile.toRelativeString(kotlincBaseDir)
            kotlincDirectory.getDirectory(path).add(FileCopy(file))
        }
    }
        ...
        for (sourcePath in sourcePaths) {
            ...
            when (sourcePath) {
                is Jar -> {
                    ...
                    val artifactName = sourcePath.project.extra.properties["ideaPluginArtifactName"]?.toString()
                            ?: sourcePath.project.name

                    val archiveForJar = Archive(artifactName + ".jar").apply {
