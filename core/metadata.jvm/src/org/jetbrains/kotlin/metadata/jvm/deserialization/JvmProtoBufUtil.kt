...
object JvmProtoBufUtil {
    ...
    /**
     * Serializes [message] and [stringTable] into two arrays of strings, which must be further written to
     * [Metadata.d1] and [Metadata.d2] correspondingly
     */
    @JvmStatic
    fun writeData(message: MessageLite, stringTable: JvmStringTable): Pair<Array<String>, Array<String>> {
        val d1 = BitEncoding.encodeBytes(ByteArrayOutputStream().apply {
            stringTable.serializeTo(this)
            message.writeTo(this)
        }.toByteArray())
        val d2 = stringTable.strings.toTypedArray()
        return Pair(d1, d2)
    }
