...
fun parse(line: String, reader: OutputLineReader, messages: MutableList<Message>): Boolean {
    ...
    if (colonIndex2 >= 0) {
        ...
        if (!file.isFile && FileUtilRt.getExtension(file.name) != "kt") {
            return addMessage(createNewMessage(getMessageKind(severity), lineWoSeverity.amendNextLinesIfNeeded(reader).trim(), null, null, null, null), messages)
        }
        ...
        if (colonIndex3 >= 0) {
            ...
            if (matcher.matches()) {
                ...
                if (lineNumber != null && symbolNumber != null) {
                    try {
                        return addMessage(createNewMessage(getMessageKind(severity), message.trim(), path, lineNumber.toInt(), symbolNumber.toInt(), symbolNumber.toInt()), messages)
                    }
            ...
            return addMessage(createNewMessage(getMessageKind(severity), message.trim(), null, null, null, null), messages)
        }
        else {
            return addMessage(createNewMessage(getMessageKind(severity), lineWoSeverity.amendNextLinesIfNeeded(reader).trim(), null, null, null, null), messages)
        }
...
private fun createNewMessage(
    ...
    return if (file == null || lineNumber == null || columnIndex == null || offset == null) {
        Message(messageKind, text, text, Optional.absent<String>(), ImmutableList.of())
    }
    else {
        ...
        Message(messageKind, text, sourceFilePosition)
    }
}
