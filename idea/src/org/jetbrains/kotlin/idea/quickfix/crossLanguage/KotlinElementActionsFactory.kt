...
class KotlinElementActionsFactory : JvmElementActionsFactory() {
    ...
    private class CreateAnnotationAction(
        ...
        override fun invoke(project: Project, editor: Editor?, file: PsiFile?) {
            ...
            val entry = target.addAnnotationEntry(KtPsiFactory(target).createAnnotationEntry("@${request.qualifiedName}${
            request.attributes.joinToString(
                ", ",
                "(",
                ")"
            ) { p -> "${p.first} = \"${(p.second as? JvmAnnotationMemberValue.StringLiteral)?.value}\"" }
            }"))
