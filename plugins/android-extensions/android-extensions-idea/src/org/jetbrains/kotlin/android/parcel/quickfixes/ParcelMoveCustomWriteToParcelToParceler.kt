...
class ParcelMoveCustomWriteToParcelToParceler(function: KtClass) : AbstractParcelableQuickFix<KtClass>(function) {
    companion object {
        ...
        private fun Project.showNotification(text: String, messageType: MessageType) {
            val statusBar = WindowManager.getInstance().getStatusBar(this) ?: return
    ...
    object FactoryForWrite : AbstractFactory({ findElement<KtClass>()?.let { ParcelMoveCustomWriteToParcelToParceler(it) } })

    object FactoryForCREATOR : AbstractFactory({
        findElement<KtObjectDeclaration>()?.getStrictParentOfType<KtClass>()?.let { ParcelMoveCustomWriteToParcelToParceler(it) }
    })
    ...
    override fun invoke(ktPsiFactory: KtPsiFactory, parcelableClass: KtClass) {
        ...
        val parcelerObjectDescriptor = bindingContext[BindingContext.CLASS, parcelerObject] ?: run {
            parcelableClass.project.showNotification(
                    "Unable to resolve parceler object for ${parcelableClass.name ?: "<unnamed Parcelable class>"}",
                    MessageType.WARNING)
            return
        }

        val parcelerTypeArg = parcelableClass.name ?: ANDROID_PARCELABLE_CLASS_FQNAME.asString()
