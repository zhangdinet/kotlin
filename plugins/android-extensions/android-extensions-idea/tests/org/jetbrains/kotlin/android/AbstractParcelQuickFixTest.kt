...
open class AbstractParcelQuickFixTest : AbstractAndroidQuickFixMultiFileTest() {
    override fun setUp() {
        ...
        val androidAll = File("dependencies/robolectric").listFiles()
                .single { it.name.startsWith("android-all-") && it.extension == "jar" }

        ConfigLibraryUtil.addLibrary(myModule, "androidJar", "dependencies/robolectric", arrayOf(androidAll.name))
