...
fun isStableValue(
    ...
    return variableDescriptor !is PropertyDescriptor || propertyKind(variableDescriptor, usageModule) === DataFlowValue.Kind.STABLE_VALUE
}

internal fun propertyKind(propertyDescriptor: PropertyDescriptor, usageModule: ModuleDescriptor?): DataFlowValue.Kind {
    if (propertyDescriptor.isVar) return DataFlowValue.Kind.MUTABLE_PROPERTY
    if (propertyDescriptor.isOverridable) return DataFlowValue.Kind.PROPERTY_WITH_GETTER
    if (!hasDefaultGetter(propertyDescriptor)) return DataFlowValue.Kind.PROPERTY_WITH_GETTER
...
internal fun variableKind(
    ...
    if (variableDescriptor !is LocalVariableDescriptor && variableDescriptor !is ParameterDescriptor) return DataFlowValue.Kind.OTHER
    if (!variableDescriptor.isVar) return DataFlowValue.Kind.STABLE_VALUE
    if (variableDescriptor is SyntheticFieldDescriptor) return DataFlowValue.Kind.MUTABLE_PROPERTY
