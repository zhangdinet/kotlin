...
object BuiltinSpecialBridgesUtil {
    @JvmStatic fun <Signature : Any> generateBridgesForBuiltinSpecial(
        ...
        isDeclarationAndDefinitionSame: (CallableMemberDescriptor) -> Boolean
    ): Set<BridgeForBuiltinSpecial<Signature>> {

        val functionHandle = DescriptorBasedFunctionHandle(function, isDeclarationAndDefinitionSame)
        val fake = !functionHandle.isDeclaration
        val overriddenBuiltin = function.getOverriddenBuiltinReflectingJvmDescriptor()!!

        val reachableDeclarations = findAllReachableDeclarations(function, isDeclarationAndDefinitionSame)
        ...
        if (fake) {
            for (overridden in function.overriddenDescriptors.map { it.original }) {
                if (!DescriptorBasedFunctionHandle(overridden, isDeclarationAndDefinitionSame).isAbstract) {
                    commonBridges.removeAll(findAllReachableDeclarations(overridden,
                                                                         isDeclarationAndDefinitionSame
        ...
        val superImplementationDescriptor =
                if (specialBridge != null && fake && !functionHandle.isAbstract)
                    findSuperImplementationForStubDelegation(function, isDeclarationAndDefinitionSame, signatureByDescriptor)
                else
                    null
...
private fun <Signature> findSuperImplementationForStubDelegation(
    ...
    val implementation = findConcreteSuperDeclaration(DescriptorBasedFunctionHandle(function, isDeclarationAndDefinitionSame)).descriptor
