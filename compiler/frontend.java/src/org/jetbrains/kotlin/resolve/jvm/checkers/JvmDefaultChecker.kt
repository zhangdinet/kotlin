...
class JvmDefaultChecker(val jvmTarget: JvmTarget) : DeclarationChecker {
    ...
    companion object {
        val JVM_DEFAULT_FQ_NAME = FqName("kotlin.annotations.JvmDefault")
    ...
    override fun check(declaration: KtDeclaration, descriptor: DeclarationDescriptor, context: DeclarationCheckerContext) {
        ...
        if (DescriptorUtils.getDirectMember(memberDescriptor) != memberDescriptor) return //skip accessors

        memberDescriptor.overriddenDescriptors.any {
            it.annotations.hasAnnotation(JVM_DEFAULT_FQ_NAME)
        }.let {
            if (it) context.trace.report(ErrorsJvm.JVM_DEFAULT_REQUIRED_FOR_OVERLOAD.on(declaration))
        }
    }
}