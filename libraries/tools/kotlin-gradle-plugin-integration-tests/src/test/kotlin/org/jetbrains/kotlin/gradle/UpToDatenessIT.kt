...
class UpToDateIT : BaseGradleIT() {
    ...
    private val compilerClasspathMutation = object : ProjectMutation {
        ...
        override fun mutateProject(project: Project) = with(project) {
            buildGradle.modify {
                val copiedFiles = originalCompilerCp.reversed().map {
                    val originalFile = File(it)
                    val newFile = File(projectDir, originalFile.nameWithoutExtension + "-1.jar")
                    originalFile.copyTo(newFile)
                    newFile.absolutePath
                }
                it.replace(originalPaths, copiedFiles.joinToString(", ") { "\'${it.replace("\\", "/")}\'" })
            }
        }
